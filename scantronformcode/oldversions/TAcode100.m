%%%%%_____TAcodenew.m GUI version    
%%%%%_____2019-09-25 (last modified, CB)
...
function varargout = TAcodenew(varargin)
% TACODENEW MATLAB code for TAcodenew.fig
%      TACODENEW, by itself, creates a new TACODENEW or raises the existing
%      singleton*.
%
%      H = TACODENEW returns the handle to a new TACODENEW or the handle to
%      the existing singleton*.
%
%      TACODENEW('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TACODENEW.M with the given input arguments.
%
%      TACODENEW('Property','Value',...) creates a new TACODENEW or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TAcodenew_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to TAcodenew_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help TAcodenew

% Last Modified by GUIDE v2.5 30-Apr-2019 13:59:37

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TAcodenew_OpeningFcn, ...
                   'gui_OutputFcn',  @TAcodenew_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before TAcodenew is made visible.
function TAcodenew_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TAcodenew (see VARARGIN)

% Choose default command line output for TAcodenew
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes TAcodenew wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TAcodenew_OutputFcn(hObject, eventdata, handles) 
...
if(isempty(answerkey))
	...
	%for iii=46
		...
		templatedistanceperp(1,1) = 246.5578;
		templatedistanceslant(1,1) = 383.8063;
		template26distanceperp = 916.6072;
		template26distanceslant = 446.2795;
		template51distanceperp = 1.5856e+03;
		template51distanceslant = 382.7536;
		template76distanceperp = 2.2536e+03;
		template76distanceslant = 446.2284;
		templateIDdistanceperp(1,1) = 2.9978e+03;
		templateIDdistanceslant(1,1) = 4.5086e+03;
		templatedistanceperpmovediagonal = 1.7192e+03;
		templatedistanceslantmovediagonal = 4.6374e+03;
		templatedistanceperpmovename = 1.1210e+03;
		templatedistanceslantmovename = 4.7801e+03;
		templatedistanceperpmovekey = 249.4069;
		templatedistanceslantmovekey = 191.8040;
		...
		for i=1:size(answerkey(1).question,2)
			...
			elseif((i>50) && (i<76))
				for j=1:numoptions
					distanceperp(i,j) = perpscale*template51distanceperp(1,1) + horizontaloffset * (j-1);
					distanceslant(i,j) = slantscale*template51distanceslant(1,1) + verticaloffset * (i-51);
			...
			elseif((i>75) && (i<101))
				for j=1:numoptions
					distanceperp(i,j) = perpscale*template76distanceperp(1,1) + horizontaloffset * (j-1);
					distanceslant(i,j) = slantscale*template76distanceslant(1,1) + verticaloffset * (i-76);
