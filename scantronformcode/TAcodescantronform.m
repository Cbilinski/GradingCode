...
%%%%%_____2019-09-25 (last modified, CB) 2020-03-04 according to last modified file system
%%%%%_____Authors: Christopher Bilinski      
%%%%%_____Contact: cgbilinsk@gmail.com   
%%%%%_____Note: 2024-02-16: The most current version of the TA code for operating on the scantron-style forms.  Image for scantron regularly changes, so there are multiple deprecated versions of this code that were created with templategenerator.m
...
function varargout = TAcodescantronform(varargin)
% TACODESCANTRONFORM MATLAB code for TAcodescantronform.fig
%      TACODESCANTRONFORM, by itself, creates a new TACODESCANTRONFORM or raises the existing
%      singleton*.
%
%      H = TACODESCANTRONFORM returns the handle to a new TACODESCANTRONFORM or the handle to
%      the existing singleton*.
%
%      TACODESCANTRONFORM('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TACODESCANTRONFORM.M with the given input arguments.
%
%      TACODESCANTRONFORM('Property','Value',...) creates a new TACODESCANTRONFORM or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before TAcodescantronform_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to TAcodescantronform_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help TAcodescantronform

% Last Modified by GUIDE v2.5 04-Mar-2020 15:18:41

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @TAcodescantronform_OpeningFcn, ...
                   'gui_OutputFcn',  @TAcodescantronform_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before TAcodescantronform is made visible.
function TAcodescantronform_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to TAcodescantronform (see VARARGIN)

% Choose default command line output for TAcodescantronform
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes TAcodescantronform wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = TAcodescantronform_OutputFcn(hObject, eventdata, handles) 
...
if(isempty(answerkey))
	...
	%for iii=46
		...
		%if switching to parallel computing, make sure to allocate each variable before using it, only a fraction of them are preallocated in the comment here
		%templatedimmovetot             = zeros(2,1,1);
		%templatedistanceperp           = zeros(1,1);
		%templatedistanceslant          = zeros(1,1);
		%templateIDdimmovetot           = zeros(2,1,1);
		%templateIDdistanceperp         = zeros(2,1,1);
		%templateIDdistanceslant        = zeros(2,1,1);
		...
		templatedistanceperp(1,1) = 250.8422;
		templatedistanceslant(1,1) = 390.1741;
		template26distanceperp = 918.8018;
		template26distanceslant = 448.6373;
		templateIDdistanceperp(1,1) = 1.5872e+03;
		templateIDdistanceslant(1,1) = 2.8291e+03;
		templatedistanceperpmovediagonal = 1.8801e+03;
		templatedistanceslantmovediagonal = 4.6407e+03;
		templatedistanceperpmovename = 956.8091;
		templatedistanceslantmovename = 4.7637e+03;
		templatedistanceperpmovekey = 250.9733;
		templatedistanceslantmovekey = 201.1741;
		
		
		%templatedistanceperpmoveteal = 2.4455e+03;
		%templatedistanceslantmoveteal = 5.1707e+03;
		...
		%find the topleft corner of the 4th black box on the top row to use as a stretch/distance measurement
		%this is found by looking at the color average of the 70 horizontal and 50 vertical pixels following the current pixel 
		%the darkest pixel is chosen as the corner
		for i=1:250
			for j=1:400
				blackshademean(i,j) = mean(mean(scannedimagegray(100+i:100+i+50,j+1800:j+1800+70)));
			end
		end
		[dim1blackhortemp dim2blackhortemp] = find(blackshademean == min(min(blackshademean)));
		dim1blackhor = min(dim1blackhortemp) + 100;
		dim2blackhor = min(dim2blackhortemp) + 1800;

		%find the 75th black bar on the left side of the scantron
		...
		for i=11:length(middlebarcol)-10
			if( (~isnan(middlebarcol(i))) & (sum(isnan(middlebarcol(i-3:i-1))) > 2) & (sum(~isnan(middlebarcol(i+1:i+3))) > 2) )
				if(abs(blackbar_rowstart(end) - i) > 15)
		...
		dim175thblackbar = blackbar_rowstart(find(abs(blackbar_rowstart - (dim1initial + mean(blackbar_rowspace) * 74)) == min(abs(blackbar_rowstart-(dim1initial + mean(blackbar_rowspace) * 74)))));
		dim275thblackbar = slantfit(2).*dim175thblackbar+slantfit(1);
		%dim175thblackbar = dim1initial + median(blackbar_rowspace) * 74
		%dim275thblackbar = slantfit(2).*dim175thblackbar+slantfit(1);
		%dim175thblackbar = blackbar_rowstart(76);
		%dim275thblackbar = slantfit(2).*blackbar_rowstart(76)+slantfit(1);

		%find the diagonal distance between the two black bar locations (to use for scaling the perp and slant size of the image)
		totaldim1movediagonal = (dim175thblackbar - dim1blackhor);
		totaldim2movediagonal = (dim275thblackbar - dim2blackhor);
		...
		%perpscale = distanceperpmoveteal / templatedistanceperpmoveteal;
		%slantscale = distanceslantmoveteal / templatedistanceslantmoveteal;

		%ABCDE offset
		%1B is at 472 549
		horizontaloffset = 84 * perpscale;

		%12345 offset
		%2A is at 387 673
		%seems like it is drifting a bit by the end, so changing the value by +1
		verticaloffset = 125 * slantscale;

		%0-0 offset for ID
		%2nd 0 is at 1790 2993
		%seems like it is drifting a bit by the end, so changing the value by -1
		%seems like it is drifting a bit by the end, so changing the value by +1
		horizontalIDoffset = 84 * perpscale;

		%0-1 offset for ID
		%1st 1 is at 1706 3055
		%seems like it is drifting a bit by the end, so changing the value by +1
		verticalIDoffset = 63 * slantscale;
		...
		for i=1:size(answerkey(1).question,2)
			...
			elseif((i>25) && (i<51))
				for j=1:numoptions
					distanceperp(i,j) = perpscale*template26distanceperp(1,1) + horizontaloffset * (j-1);
					distanceslant(i,j) = slantscale*template26distanceslant(1,1) + verticaloffset * (i-26);
			...
			for ii=1:numoptions
				for i=1:25
					for j=1:25
						...
						scannedimageRGB(dim175thblackbar-13+i,round(dim275thblackbar)-13+j,1) = 255;
						scannedimageRGB(dim175thblackbar-13+i,round(dim275thblackbar)-13+j,2)= 0;
						scannedimageRGB(dim175thblackbar-13+i,round(dim275thblackbar)-13+j,3) = 0;
						for jj=1:4
							keyloc(1,jj)-13+i;
...
meangradefrac = meangrade/size(answerkey(1),2);
